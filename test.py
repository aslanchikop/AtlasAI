...
/* Status indicator - online (green blinking dot) */
.status-online {
    display: inline-block;
    width: 8px;
    height: 8px;
...
/* Data card with subtle animation */
.data-card {
    transition: all 0.3s ease;
...
/* CRITICAL: Fix all inline HTML text visibility */
.stMarkdown div, .stMarkdown span, .stMarkdown p,
...
/* Fix formulas and code in encyclopedia */
...
import random
if random.random() < 0.05:  # 5% chance per interaction
    toast_messages = {
        'ru': [
            "📡 Спутник TESS передал новый пакет данных",
            "🔭 Телескоп Kepler завершил калибровку",
            "⚡ Зафиксирована активность на Проксиме Центавра",
            "🛰️ Получен сигнал от зонда Voyager 1",
            "📊 Обновлена база данных NASA Exoplanet Archive",
            "🌟 Обнаружен транзит в системе TOI-700",
        ],
        'en': [
            "📡 TESS satellite transmitted new data packet",
            "🔭 Kepler telescope calibration complete",
            "⚡ Activity detected on Proxima Centauri",
            "🛰️ Signal received from Voyager 1 probe",
            "📊 NASA Exoplanet Archive database updated",
            "🌟 Transit detected in TOI-700 system",
        ]
    }
    lang = st.session_state.get('lang', 'ru')
...
with st.sidebar:
    # ATLAS Header with status
    st.markdown("""
    <div class='atlas-header' style='background: linear-gradient(135deg, rgba(0,20,40,0.9), rgba(0,50,80,0.8));
                border-radius: 16px; padding: 20px; margin-bottom: 15px; text-align: center;'>
        <div style='font-size: 2rem;'>🛰️</div>
        <h2 style='margin: 5px 0; font-family: Orbitron, monospace; color: #00d4ff;'>ATLAS</h2>
        <div style='font-size: 0.75rem; opacity: 0.7;'>Autonomous Terrestrial Life Analysis System</div>
        <div style='margin-top: 10px;'>
            <span class='status-online'></span>
            <span style='font-size: 0.8rem; color: #00ff88;'>ONLINE</span>
        </div>
    </div>
    ...
    <div style='text-align: center; margin-bottom: 15px;'>
        <div style='font-size: 0.75rem; opacity: 0.6;'>⏱️ MISSION ELAPSED TIME</div>
        <div class='mission-timer' style='font-size: 1.2rem; font-weight: bold;'>
    ...
    # AI Model Status with animation
    st.markdown("### 🧠 AI Model")
    if EXOPLANET_MODEL_AVAILABLE:
        st.markdown("""
        <div style='display: flex; align-items: center; gap: 8px;'>
            <span class='status-online'></span>
            <span style='color: #00ff88;'>Model Active</span>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
        <div style='display: flex; align-items: center; gap: 8px;'>
            ...
            <span style='color: #ff6666;'>Model Offline</span>
        </div>
        """, unsafe_allow_html=True)
        st.caption("Add exoplanet_model.pkl")
    
    # Statistics with live indicator
...
with tabs[1]:
    if 'planets' in st.session_state and st.session_state['planets']:
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # Используем HTML-теги <strong> вместо ** для выделения внутри HTML-блока
        atlas_greeting = {
            'ru': f"Система <strong style='color: {accent};'>{hostname}</strong> загружена. Обнаружено <strong style='color: {accent};'>{len(planets)}</strong> планет. Провожу глубокий анализ...",
            'en': f"System <strong style='color: {accent};'>{hostname}</strong> loaded. <strong style='color: {accent};'>{len(planets)}</strong> planets detected. Running deep analysis...",
            'kz': f"<strong style='color: {accent};'>{hostname}</strong> жүйесі жүктелді. <strong style='color: {accent};'>{len(planets)}</strong> планета табылды. Терең талдау жүргізілуде..."
        }
        
        st.markdown(f"""
        <div style='background: linear-gradient(135deg, {card_bg}, rgba(0,100,150,0.2));
                    border: 2px solid {accent}; border-radius: 20px; padding: 25px; margin-bottom: 20px;
                    box-shadow: 0 0 30px rgba(0,212,255,0.2);'>
            <div style='display: flex; align-items: center; gap: 20px;'>
                <div style='font-size: 3.5rem; text-shadow: 0 0 20px {accent};'>💠</div>
                <div>
                    <h2 style='margin: 0; color: {accent}; font-family: Orbitron, monospace; text-transform: uppercase; letter-spacing: 2px;'>ATLAS AI Core</h2>
                    <p style='margin: 8px 0 0 0; color: {text_dim}; font-size: 1.1rem; line-height: 1.5;'>{atlas_greeting[lang]}</p>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Подготовка переменных для таблицы (защита от ошибок форматирования)
        lum_text = f"{st_lum:.4f}" if st_lum else "?"
        age_text = f"{st_age:.2f} Gyr" if st_age else "?"
        met_text = f"{st_met:+.2f}" if st_met else "?"
        ...
        with star_cols[1]:
            # Star parameters (Исправлены отступы и центрирование)
            st.markdown(f"""
            <div style='background: {card_bg}; border: 1px solid {card_border}; border-radius: 16px; padding: 20px;'>
                ...
                <table style='width: 100%; color: {text_main}; border-collapse: collapse;'>
                    <tr style='border-bottom: 1px solid rgba(255,255,255,0.05);'>
                        <td style='padding: 12px 15px; color: {text_dim}; text-align: left;'>🌡️ {lbl_temp}</td>
                        <td style='padding: 12px 15px; font-weight: bold; text-align: center;'>{st_teff:,.0f} K</td>
                        <td style='padding: 12px 15px; color: {text_dim}; font-size: 0.85rem; text-align: left;'>(☀️ 5778K)</td>
                    </tr>
                    <tr style='border-bottom: 1px solid rgba(255,255,255,0.05);'>
                        <td style='padding: 12px 15px; color: {text_dim}; text-align: left;'>📏 {lbl_rad}</td>
                        <td style='padding: 12px 15px; font-weight: bold; text-align: center;'>{st_rad:.3f} R☉</td>
                        <td style='padding: 12px 15px; color: {text_dim}; font-size: 0.85rem; text-align: left;'>({st_rad * 696340:.0f} km)</td>
                    </tr>
                    <tr style='border-bottom: 1px solid rgba(255,255,255,0.05);'>
                        <td style='padding: 12px 15px; color: {text_dim}; text-align: left;'>⚖️ {lbl_mass}</td>
                        <td style='padding: 12px 15px; font-weight: bold; text-align: center;'>{st_mass:.3f} M☉</td>
                        <td style='padding: 12px 15px; color: {text_dim}; font-size: 0.85rem; text-align: left;'>({st_mass * 1.989e30:.2e} kg)</td>
                    </tr>
                    <tr style='border-bottom: 1px solid rgba(255,255,255,0.05);'>
                        <td style='padding: 12px 15px; color: {text_dim}; text-align: left;'>💡 {lbl_lum}</td>
                        <td style='padding: 12px 15px; font-weight: bold; text-align: center;'>{lum_text} L☉</td>
                        <td style='padding: 12px 15px; color: {text_dim}; font-size: 0.85rem; text-align: left;'>{lum_compare}</td>
                    </tr>
                    <tr style='border-bottom: 1px solid rgba(255,255,255,0.05);'>
                        <td style='padding: 12px 15px; color: {text_dim}; text-align: left;'>🕐 {lbl_age}</td>
                        <td style='padding: 12px 15px; font-weight: bold; text-align: center;'>{age_text}</td>
                        <td style='padding: 12px 15px; color: {text_dim}; font-size: 0.85rem; text-align: left;'>(☀️ 4.6 Gyr)</td>
                    </tr>
                    <tr>
                        <td style='padding: 12px 15px; color: {text_dim}; text-align: left;'>🧪 {lbl_met}</td>
                        <td style='padding: 12px 15px; font-weight: bold; text-align: center;'>{met_text} [Fe/H]</td>
                        <td style='padding: 12px 15px; color: {text_dim}; font-size: 0.85rem; text-align: left;'>(☀️ 0.00)</td>
                    </tr>
                </table>
            </div>
            """, unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div style='background: linear-gradient(90deg, rgba(0,150,100,0.2), rgba(0,100,200,0.2));
        ...
        with c5:
            ...
            if sel_p.get('ml_probability') is not None:
                ...
                prob_pct = prob * 100
                prob_color = '#00ff88' if prob >= 0.7 else '#00d4ff' if prob >= 0.5 else '#ffbb00' if prob >= 0.3 else '#ff6666'
                
                st.markdown(f"""
                <div style='background: linear-gradient(135deg, rgba(100,0,200,0.2), rgba(0,100,200,0.1));
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 4: COMPARE (FIXED TABLE with HTML styling and Centered Text)
# ═══════════════════════════════════════════════════════════════════════════════
with tabs[4]:
    ...
    if len(available_planets) >= 2:
        ...
        if selected_planets and len(selected_planets) >= 2:
            ...
            # Строим таблицу СТРОГО в одну строку (без """ и переносов), чтобы не ломался Markdown
            # + Добавлено text-align: center для колонок с цифрами
            html = f'<table style="width: 100%; border-collapse: collapse; margin: 16px 0; font-family: \'Inter\', sans-serif;">'
            html += '<thead><tr>'
            html += f'<th style="background: {bg_header}; color: {text_color}; padding: 14px 12px; text-align: left; font-weight: 600; border-bottom: 2px solid {border_accent};">🪐 Planet</th>'
            html += f'<th style="background: {bg_header}; color: {text_color}; padding: 14px 12px; text-align: center; font-weight: 600; border-bottom: 2px solid {border_accent};">Radius (R⊕)</th>'
            html += f'<th style="background: {bg_header}; color: {text_color}; padding: 14px 12px; text-align: center; font-weight: 600; border-bottom: 2px solid {border_accent};">Mass (M⊕)</th>'
            html += f'<th style="background: {bg_header}; color: {text_color}; padding: 14px 12px; text-align: center; font-weight: 600; border-bottom: 2px solid {border_accent};">Temp (K)</th>'
            html += f'<th style="background: {bg_header}; color: {text_color}; padding: 14px 12px; text-align: center; font-weight: 600; border-bottom: 2px solid {border_accent};">ESI</th>'
            html += f'<th style="background: {bg_header}; color: {text_color}; padding: 14px 12px; text-align: center; font-weight: 600; border-bottom: 2px solid {border_accent};">Gravity (g)</th>'
            html += f'<th style="background: {bg_header}; color: {text_color}; padding: 14px 12px; text-align: center; font-weight: 600; border-bottom: 2px solid {border_accent};">Distance (ly)</th>'
            html += '</tr></thead><tbody>'
            ...
            for name in selected_planets:
                ...
                html += '<tr>'
                html += f'<td style="background: {bg_cell}; color: {text_accent}; padding: 12px; border-bottom: 1px solid {border_color}; font-weight: 600; text-align: left;">{emoji} {name}</td>'
                html += f'<td style="background: {bg_cell}; color: {text_color}; padding: 12px; border-bottom: 1px solid {border_color}; text-align: center;">{radius}</td>'
                html += f'<td style="background: {bg_cell}; color: {text_color}; padding: 12px; border-bottom: 1px solid {border_color}; text-align: center;">{mass}</td>'
                html += f'<td style="background: {bg_cell}; color: {text_color}; padding: 12px; border-bottom: 1px solid {border_color}; text-align: center;">{temp}</td>'
                html += f'<td style="background: {bg_cell}; color: {text_color}; padding: 12px; border-bottom: 1px solid {border_color}; text-align: center;">{esi}</td>'
                html += f'<td style="background: {bg_cell}; color: {text_color}; padding: 12px; border-bottom: 1px solid {border_color}; text-align: center;">{gravity}</td>'
                html += f'<td style="background: {bg_cell}; color: {text_color}; padding: 12px; border-bottom: 1px solid {border_color}; text-align: center;">{distance}</td>'
                html += '</tr>'
...
with tabs[8]:
    ...
    if CHATBOT_AVAILABLE:
        ...
        st.info("⚡ Pattern matching mode — Place exo_chatbot_model.pkl in app folder for ML")
